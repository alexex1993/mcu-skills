...
build_flags =
    -DHSE_VALUE=25000000                  ; X1 is 25 MHz, not the CMSIS default
    -Wall
...
**`-DHSE_VALUE=25000000` is not optional.** CMSIS's `system_stm32h7xx.c` defaults `HSE_VALUE` to
25 MHz for some product lines and 8 MHz for others; if it lands on 8 MHz, `HAL_RCC_GetSysClockFreq()`
and everything derived from it (`SystemCoreClock`, `HAL_Delay`, UART baud dividers, timer periods)
are wrong by a factor of ~3 while the PLL itself still locks — so the board runs, just with every
timing silently off.
...
/* PA11 = DM, PA12 = DP, GPIO_AF10_OTG1_FS */
```
...
| Symptom | Cause | Fix |
|---|---|---|
| Every timing off by ~3×, PLL still locks | `HSE_VALUE` defaulted to 8 MHz | `-DHSE_VALUE=25000000` |
