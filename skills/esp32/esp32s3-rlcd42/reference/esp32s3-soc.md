...
**Neither half is used on this board.** The reflective panel is a 1-bpp SPI device on
SPI2 — a full frame is 15,000 bytes, so there is nothing for LCD_CAM or its DMA to earn.
Both halves are free if you wire a camera or a parallel panel to the expansion header,
but the header exposes three free GPIOs and DVP needs fourteen.
...
| SPI | SPI2 (FSPI) + SPI3 | SPI2's IO_MUX pads are GPIO9-14. **This board's LCD is on SPI2 via GPIO11/12, which are IO_MUX pads for SCLK/MOSI** — but CS lands on GPIO40, so the transaction is matrix-routed anyway and the ~40 MHz ceiling applies |
...
| LEDC | 8 channels, 4 timers | nothing on this board claims one — there is no backlight and no LED to dim |
| PCNT | 4 units | |
| TWAI | 1 | CAN 2.0 |
...
On this board that leaves **GPIO0 (BOOT), GPIO18 (KEY), GPIO1, GPIO2, GPIO3 and GPIO17**
as candidate wake pins. The two buttons are the useful ones: both idle high through an
internal pull-up and pull low when pressed, so `ext0` with a level of 0 needs no extra
hardware. Note that the panel keeps its image through deep sleep — see `recipes.md` §9.
...
| XTAL_32K on GPIO15/16 | optional 32.768 kHz — **GPIO15 is the RTC interrupt and GPIO16 is I2S MCLK here, so neither is available.** The board solves the same problem with a PCF85063A on I2C instead |
...
capacity — worth knowing if the board runs hot in its enclosure.

Cache and DMA: the S3's cache is coherent for GDMA on internal SRAM but PSRAM access goes
through the same cache lines the CPU uses. Nothing on this board does large DMA; hand-
